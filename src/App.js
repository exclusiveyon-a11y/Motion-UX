...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .step-dot {
    ...
    background: #E0DED8; transition: all .25s;
  }
  ...
  /* 링 */
  .ring {
    position: absolute; border-radius: 50%; border: 1.5px solid #0A0A0A;
  ...
  /* ── 검색 입력 ── */
  .input-card {
    background: #F5F3EE; border-radius: 16px;
    border: 1px solid #E0DED8; margin-bottom: 16px;
  ...
  .dest-input {
    ...
    outline: none; width: 100%; padding: 0; color: #0A0A0A;
  }
  ...
  .search-drop {
    ...
    background: #fff; border: 1px solid #E0DED8; border-radius: 14px;
    z-index: 200; box-shadow: 0 8px 24px rgba(0,0,0,.10); overflow: hidden;
  }
  ...
  .toggle-pill.on  .t-knob { left: 21px; }
  .toggle-pill.off .t-knob { left: 3px; }
  ...
  .toggle-sm.on  .sm-knob { left: 15px; }
  .toggle-sm.off .sm-knob { left: 3px; }
  ...
  .s-name { font-size: 13px; font-weight: 500; }
  .s-val  { font-size: 11px; color: #888; margin-top: 2px; }
  ...
  .g-row { display: flex; align-items: flex-start; gap: 10px; margin-bottom: 8px; }
...
function calcMSDV(pts) {
  ...
  for (let i = 1; i < pts.length - 1; i++) {
    const [p1,p2,p3] = [pts[i-1],pts[i],pts[i+1]];
...
function usePlaces() {
  ...
  const search = useCallback((q) => {
    ...
    t.current = setTimeout(() => {
      ...
      new window.kakao.maps.services.Places().keywordSearch(q, (data, st) => {
        if (st === window.kakao.maps.services.Status.OK)
          setResults(data.slice(0,5).map(r=>({ name:r.place_name, addr:r.road_address_name||r.address_name, lat:+r.y, lng:+r.x })));
        else setResults([]);
      });
    }, 250);
  }, []);
...
// ─── KakaoMap 컴포넌트 ───
function KakaoMap({ center, zoom=5, routes=[], markers=[], height=220 }) {
  ...
  useEffect(() => {
    ...
    map.current.setCenter(new window.kakao.maps.LatLng(center.lat, center.lng));
  }, [center.lat, center.lng]);

  useEffect(() => {
    if (!map.current || !window.kakao?.maps) return;
    const K = window.kakao.maps;
    ovl.current.forEach(o => o.setMap(null));
    ovl.current = [];

    routes.forEach(r => {
      if (!r.points?.length) return;
      const poly = new K.Polyline({
        path: r.points.map(p => new K.LatLng(p.y, p.x)),
        strokeWeight: r.active ? 6 : 2,
        strokeColor:  r.active ? "#0A0A0A" : "#BFBDB4",
        strokeOpacity: r.active ? 1 : 0.4,
        strokeStyle: "solid",
      });
      poly.setMap(map.current);
      ovl.current.push(poly);
    });

    markers.forEach(m => {
      const mk = new K.Marker({ position: new K.LatLng(m.lat, m.lng), map: map.current });
      ovl.current.push(mk);
    });
    ...
    if (allPts.length) {
      ...
      allPts.forEach(p => b.extend(new K.LatLng(p.y, p.x)));
      map.current.setBounds(b, 40);
    }
  }, [routes, markers]);
...
export default function App() {
  ...
  const onQueryChange = q => { setQuery(q); setDest(null); if(kakao) search(q); };
  ...
  const mapMarkers = dest ? [{ lat:loc.lat, lng:loc.lng },{ lat:dest.lat, lng:dest.lng }] : [];
    ...
    <>
      ...
      <div className="app">
          ...
          {step > 1
            ? <button className="back-btn" onClick={()=>setStep(s=>s-1)}>
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                  <path d="M10 3L5 8l5 5" stroke="#0A0A0A" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </button>
            : <div style={{width:32}}/>
          }
        ...
        <div className="content">
          ...
          {step===1 && <>
            {kakao
              ? <KakaoMap center={loc} zoom={5} markers={[{lat:loc.lat,lng:loc.lng}]} height={240} />
                    ...
                    <div className="search-drop">
                      {results.map(r=>(
                        <div key={r.name+r.addr} className="s-item" onClick={()=>onPickDest(r)}>
                          ...
                          <div className="s-addr">{r.addr}</div>
                        </div>
                      ))}
                    </div>
                  )}
          ...
          {step===2 && <>
            {loading
              ...
              : <>
                  {kakao && routes && routes[0].points.length>0
                    ? <KakaoMap center={dest||loc} zoom={6} routes={mapRoutes} markers={mapMarkers} height={200}/>
          ...
          {step===3 && <div className="pad">
            ...
            <div className={`items-card${masterOn?"":" off"}`}>
              {SETTINGS.map((s,i)=>(
                <div key={i} className="s-row">
                  ...
                  <div className={`toggle-sm ${rowOn[i]?"on":"off"}`} onClick={()=>setRowOn(r=>r.map((v,j)=>j===i?!v:v))}>
                    <div className="sm-knob"/>
                  </div>
                </div>
              ))}
            </div>
            <div className="preview-box">
              ...
              <div className="pills">
                {SETTINGS.map((s,i)=>(
                  <span key={i} className={`p-pill${masterOn&&rowOn[i]?"":" off"}`}>{s.tag}</span>
                ))}
              </div>
            </div>
          </div>}
          ...
          {step===4 && <>
            {kakao && dest
              ? <KakaoMap center={dest} zoom={4} markers={[{lat:dest.lat,lng:dest.lng}]} height={220}/>
            ...
            <div className="pad">
              ...
              <div className="setting-status">
                ...
                <div className="pills">
                  {SETTINGS.filter((_,i)=>rowOn[i]).map((s,i)=><span key={i} className="pill">{s.tag}</span>)}
                </div>
              </div>
              <div className="route-summary-row">
                <div>
                  <div style={{fontSize:13,fontWeight:600}}>{rd.name}</div>
                  <div style={{fontSize:11,color:"#888",marginTop:2}}>{rd.time} 예상 · {rd.badge}</div>
                </div>
            ...
            <div className="pad">
              ...
              <div className="ready-row">
                <div className="ready-icon">
                  <svg width="13" height="13" viewBox="0 0 12 12" fill="none"><path d="M2 6.5l2.5 2.5 5.5-5.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                </div>
